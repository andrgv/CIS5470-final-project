...
// Overflow Analysis chaotic iteration implementation
void OverflowAnalysis::flowIn(Instruction *Inst, Memory *InMem) {
  std::vector<Instruction *> Preds = getPredecessors(Inst);
...
void OverflowAnalysis::doAnalysis(Function &F) {
  ...
  while (!WorkSet.empty()) {
    ...
    if (iteration_count > MAX_ITERATIONS) {
      errs() << "WARNING: Overflow analysis hit iteration limit ("
             << MAX_ITERATIONS << ") - forcing convergence\n";
      break;
    }
    ...
    OverflowAnalysis::transfer(Inst, InMem, *Out);
    flowOut(Inst, OutMap[Inst], Out, WorkSet);
  }
}
