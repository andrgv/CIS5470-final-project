...
entry:
  ...
  store i32 100, i32* %num_imgs, align 4
  %0 = load i32, i32* %num_imgs, align 4
  %cmp = icmp sgt i32 %0, 0
  br i1 %cmp, label %land.lhs.true, label %if.else

land.lhs.true:                                    ; preds = %entry
  %1 = load i32, i32* %num_imgs, align 4
  %cmp1 = icmp sle i32 %1, 209715
  br i1 %cmp1, label %if.then, label %if.else

if.then:                                          ; preds = %land.lhs.true
  %2 = load i32, i32* %num_imgs, align 4
  %conv = sext i32 %2 to i64
  %mul = mul i64 10240, %conv
  %call = call noalias i8* @malloc(i64 noundef %mul) #2
  %3 = bitcast i8* %call to %struct.img_t*
  store %struct.img_t* %3, %struct.img_t** %table_ptr, align 8
  br label %if.end

if.else:                                          ; preds = %land.lhs.true, %entry
  store %struct.img_t* null, %struct.img_t** %table_ptr, align 8
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then
  %4 = load %struct.img_t*, %struct.img_t** %table_ptr, align 8
  %tobool = icmp ne %struct.img_t* %4, null
  br i1 %tobool, label %if.then2, label %if.end4

if.then2:                                         ; preds = %if.end
  %5 = load %struct.img_t*, %struct.img_t** %table_ptr, align 8
  %arrayidx = getelementptr inbounds %struct.img_t, %struct.img_t* %5, i64 0
  ...
  %6 = load %struct.img_t*, %struct.img_t** %table_ptr, align 8
  %7 = bitcast %struct.img_t* %6 to i8*
